...
def resize_image(img, H, W, interpolation=cv2.INTER_LINEAR):
...
def get_intrinsics(basedir, crop, use_filtered_depth=True):
    if use_filtered_depth:
        depth = imageio.imread(os.path.join(basedir, 'depth_filtered', 'depth0.png'))
    else:
        depth = imageio.imread(os.path.join(basedir, 'depth', '0.png'))
    H, W = depth.shape[:2]
    H = H - crop / 2
    W = W - crop / 2
...
def load_scannet_data(basedir, trainskip, downsample_factor=1, translation=0.0, sc_factor=1., crop=0, use_filtered_depth=True):
    depth_folder_name = 'depth_filtered' if use_filtered_depth else 'depth'
    
    # Get image filenames, poses and intrinsics
    img_files = [f for f in sorted(os.listdir(os.path.join(basedir, 'images')), key=alphanum_key) if f.endswith('png') or f.endswith('jpg')]
    
    
    depth_files = [f for f in sorted(os.listdir(os.path.join(basedir, depth_folder_name)), key=alphanum_key) if f.endswith('png')]
    ...
    for i in train_frame_ids:
        if valid_poses[i]:
            ...
            depth = imageio.imread(os.path.join(basedir, depth_folder_name, depth_files[i]))
                        
            img = np.array(img).astype(np.float32)
            depth = np.array(depth).astype(np.float32)
            
            H, W = np.array(depth).shape[:2]
                               
            images.append(resize_image(img, H, W))
            depth_maps.append(depth)
            poses.append(all_poses[i])
            frame_indices.append(i)

    # Map images to [0, 1] range
    images = (np.stack(images, axis=0) / 255.).astype(np.float32)

    # Convert depth to meters, then to "network units"
    depth_shift = 1000.0
    depth_maps = (np.stack(depth_maps, axis=0) / depth_shift).astype(np.float32)
